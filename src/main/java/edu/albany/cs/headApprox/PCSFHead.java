 ...
 * Head approximation
 * 
 * Aglorithm 3 of "http://people.csail.mit.edu/ludwigs/papers/icml15_graphsparsity.pdf"
 * Title : "A Nearly-Linear Time Framework for Graph-Structured Sparsity"
 * Authors : Ludwig Schmidt, Chinmay Hegde, and Piotr Indyk
 * Coded by Baojian
 *
 * @author baojian
 */
public class PCSFHead {
    ...
    public PCSFHead(ArrayList<Integer[]> edges, ArrayList<Double> c, ArrayList<Double> pi, int g, double C) {
        ...
        this.C = C;
        this.g = g;
        this.delta = 1.0D / 1609.0D; // By Theorem 11 in this paper (p.20).
     ...
     * @param B
     */
    public PCSFHead(ArrayList<Integer[]> edges, ArrayList<Double> edgeCostsW, double[] b, int s, int g, double B, int[] trueSubGraph) {

        this.edges = edges;
        this.c = new ArrayList<Double>();                            // let c(e) = w(e) + (B / s)
        for (double w : edgeCostsW) {
            this.c.add(w + B / (s * 1.0D));
        }
        this.pi = new ArrayList<Double>();                            // let pi(i) = bi*bi
        for (int i = 0; i < b.length; i++) {
            this.pi.add(b[i] * b[i]);
        }
        this.C = 2.0D * B;                                            // let C  = 2*B
        this.delta = 1.0D / 169.0D;
        this.cH = Math.sqrt(1.0D / 14.0D);

        this.g = g;
        this.trueSubGraph = trueSubGraph;
        this.trueSubGraph = null; //TODO we skip the validation of equation 9
        if (verboseLevel >= 1) {
            //------------------------------------------------------------------------------------
    ...
    /**
     * check input validation, to make sure the following properties :
     * 1. the graph is connected.
     * 2. the graph does not have self cyclic edge
     *
     * @param edges
     * @param pi
     */
    ...
    private F run() {
        ...
        if (forest.costF <= 2.0D * this.C) {    // Ensure that we have the invariant c(Fr) > 2C
            return forest;
        }
        ...
        int iter = 0;
        while (lambdaR.subtract(lambdaL).compareTo(epsilon) == 1) {// Binary search over the Lagrange parameter lambda
            ...
            if (verboseLevel >= 1) {
                ...
                System.out.println(" epsilon : " + epsilon + "lambda R: " + lambdaR + "lambda L : " + lambdaL + "lambda M : " + lambdaM);
            }
        }

        if (verboseLevel >= 1) {
            System.out.println("final lambdaL : " + lambdaL.doubleValue() + " final lambdaR : " + lambdaR.doubleValue());
        ...
        if (verboseLevel >= 1) {
            System.out.println("# nodes in L : " + forestL.nodesInF.size() + "# edges in L :" + forestL.edgesInF.size() + " # CC : " + forestL.gamma);
            System.out.println("# nodes in R : " + forestR.nodesInF.size() + "# edges in R :" + forestR.edgesInF.size() + " # CC : " + forestR.gamma);
        }

        F forestRPrime = this.pruneForest(forestR, this.c, this.pi, this.C);    // Prune the potentially large solution Fr
    ...
    /**
     * we need to check the validation of our result.
     * We assume the norm ||b-bSPrime|| is the minimum one. (This means you need to know which subset is optimal)
     *
     * @return true the equation 9 is satisfied. ; if it returns false, the algorithm must have error(s).
     */
    private boolean checkEqu9Valid(double[] b) {
        if (this.trueSubGraph == null) { // do not need to check equation 9
            return true;
        }
        ...
        if (leftNorm >= this.cH * rightNorm) {
            //System.out.println(leftNorm + ">=" + this.cH*rightNorm);
            flag = true;
        } else {
            //System.out.println("Error : "+leftNorm + ">=" + this.cH*rightNorm);
            flag = false;
        }
        return flag;
    }
    ...
    private F pruneForest(F forest, ArrayList<Double> edgeCosts, ArrayList<Double> prizes, double costBudget) {
        ...
        for (int i = 0; i < forest.size(); i++) {
            ...
            if (costBudgetR >= treei.costTree) {
                ...
                costBudgetR = costBudgetR - treei.costTree;    // cost budget C^i = c(Ti)

            } else if (costBudgetR > 0) {
                treePrimei = this.pruneTree(treei, edgeCosts, prizes, costBudgetR);
                treePrimes.add(treePrimei);
                costBudgetR = 0.0D;                            // cost budget C^i = Cr
            } else {
                treePrimei = treei.getOneNodeMaxTree();
                treePrimes.add(treePrimei);                    // cost budget C^i = 0
            }
        }
    ...
    private Tree pruneTree(Tree tree, ArrayList<Double> edgeCosts, ArrayList<Double> pi, double CPrime) {
        ArrayList<Integer> tourL = tree.getEulerTour();    //T = (Vt, Et), the method of getTour has beend tested
        ...
        for (int j = 0; j < tourL.size(); j++) {
            ...
            if (hashSet.add(nodeI)) {            //if position j is the first appearance of vj in L
        ...
        for (int i = 0; i < tourL.size(); i++) {
            ...
            if (Pl.size() >= 2) { // make sure Pl has at least two nodes, which means it has at least one edge
            ...
            if (Pl.size() >= 1) { // make sure Pl has at least one node, any path has at leat one node
            ...
            if (CPrimeInPl > CPrime) {
                ...
                if (Pl.size() == 1) { // only one node
                    ...
                    return new Tree(currentNode, pi.get(currentNode)); //create one node tree
                } else if (Pl.size() >= 2) {
                    ...
                    for (int ii = 0; ii < Pl.size() - 1; ii++) {
                        ...
                        if ((flag0 == false) && (flag1 == false)) {    //add new edge
                    ...
                    return new Tree(nodesInT, edgesInT, edgesCostInT, prizePiInT);    // return the subtree of T on the nodes in Pl
                } else {
        ...
        this.mergePNodes(Pl, PlPlus1, l); // algorithm will never reach this point
        return null;
    }
     ...
     *
     * @param edges     the graph G
     * @param edgeCosts the edge costs in graph
     * @param pi        prizes
    ...
    public F PCSF_GW(ArrayList<Integer[]> edges, ArrayList<Double> edgeCosts, ArrayList<Double> pi, int g) {
        ...
        for (int i : nodesInF) {
            piInF.add(this.pi.get(i));            //be careful : the prize should be the original prize not local variable pi
        }
        ...
        F forest = new F(nodesInF, edgesInF, costsInF, piInF, totalPrizes);    //the nodes and prizes, edges and costs are consistent
     ...
     *
     * @param edges
     * @return true is the graph is connected , false : the graph is not connected
     */
     ...
     * @return
     */
    public ArrayList<ArrayList<Integer>> transform2Adj(ArrayList<Integer[]> edges) {
        HashSet<Integer> nodes = new HashSet<Integer>();
    ...
    /**
     * The forest represents the subset of edges found by algorithm,
     * The forest can be seen as a combination of several trees
     *
     * @author baojian
     */
    public class F {
        ...
        public final ArrayList<Tree> trees; // we construct corresponding trees of forest

        /**
         * make sure the nodes and prizes, edges and costs are correspondingly consistent,
         * which means the edges and nodes do not start from 0 ...
         *
         * @param nodesInF
         * @param edgesInF
         * @param edgesCostInF
         ...
         * @param totalPrizes
         */
        public F(ArrayList<Integer> nodesInF, ArrayList<Integer[]> edgesInF,
                 ArrayList<Double> edgesCostInF, ArrayList<Double> prizePiInF, double totalPrizes) {
            ...
            if (this.gamma != this.trees.size()) { //make sure the gamma is the number of trees in this forest
        ...
        private ArrayList<Tree> constructTrees() {
            ...
            HashMap<Integer, Set<Integer>> componentsMap = dis.getConnectedComponents(); // all of components in F
            ArrayList<Tree> trees = new ArrayList<Tree>();
            for (Integer key : componentsMap.keySet()) { // for each component, create a new tree
                Tree tree;
                Set<Integer> nodes = componentsMap.get(key); // nodes in this component
                ArrayList<Integer> nodesInT = new ArrayList<Integer>(nodes); // get all nodes in a tree
                ArrayList<Integer[]> edgesInT = new ArrayList<Integer[]>();
                ArrayList<Double> edgesCostInT = new ArrayList<Double>();
                ArrayList<Double> prizePiInT = new ArrayList<Double>();
                for (Integer[] edge : this.edgesInF) {    // get all edges in a tree
                    if (nodes.contains(edge[0]) || nodes.contains(edge[1])) {
                        edgesInT.add(edge);
                        int index = this.edgesInF.indexOf(edge);
                        edgesCostInT.add(this.edgesCostInF.get(index));    // get all edge cost in a tree
                    }
                }
        ...
        /**
         * using disjoint set data structure to find number of connected component in a forest
         *
        ...
        /**
         * sort the trees in this forest by descending ratio
         * r1 > r2 > r3 > ...
         *
         * @return the sorted trees
         */
        public ArrayList<Tree> getDescendingSortedTrees() {
            Collections.sort(this.trees, new Comparator<Tree>() {    // with descending order
                //@Override
        ...
        /**
         * @return the size of the trees in Forest, or we can say it is the value of gamma function.
         */
        public int size() {
    ...
    public class Tree implements Comparable<Tree> {
        ...
        public final ArrayList<Integer[]> edgesInT;    // Note : edges in T are undirected. Therefore, each of edge only save once.
         ...
         * @param prizePi
         */
        public Tree(ArrayList<Integer> nodesInT, ArrayList<Integer[]> edges,
                    ArrayList<Double> edgesCost, ArrayList<Double> prizePi) {
        ...
        /**
         * we simulate a tree as directed graph, and each edge in the tree are simulated as two edges with different edge direction.
         * For example, an edge [i,j] in this tree will be simulated as [i->j] and [j->i].
         * Therefore, we can construct a connected Directed Graph. After that, we can find a tour trough depth first search algorithm.
         *
